...
\vspace{0.4cm}
\hspace{2cm}\textbf{Mentor:} {[}Ime i prezime nastavnika{]}\par
\end{flushleft}
...
\begin{itemize}
  ...
  \item ostvariti ponovljivu pripremu podataka — sirovi izvori i obrađene datoteke pohranjeni su u repozitoriju
\end{itemize}
...
\begin{itemize}
  ...
  \item vrijednost se sprema u lokalnu predmemoriju (\texttt{scripts/.cache/youtube-views.json}) te se zapisuje u konačnu datoteku \texttt{entries.json}
\end{itemize}
...
Obrada se odvija u Node.js skripti \texttt{scripts/build-data.js}. Koraci su sljedeći:
...
\begin{enumerate}
  ...
  \item preslikavanje dvoslovne oznake zemlje (npr.\ ,,hr'') u ISO Alpha-3 (npr.\ ,,HRV'') pomoću statičke mape u \texttt{country-codes.js}; postoji i pričuvno preslikavanje \texttt{NAME\_TO\_ISO2} jer pojedini redovi sadrže puno ime umjesto oznake
  ...
  \item zaseban prolaz \texttt{scripts/fetch-youtube-views.js} dohvaća broj pregleda uz ograničenje od 800\,ms između zahtjeva i upisuje vrijednost u polje \texttt{youtubeViews}
\end{enumerate}
...
Različitih zemalja & 45 \\
Brojeva pregleda na YouTubeu & 276 (za 12 nije dohvaćeno) \\
...
\begin{itemize}
  ...
  \item Postoji li geografski uzorak — preferiraju li određene regije određene zemlje?
  ...
  \item Koje se zemlje najčešće nisu plasirale u finale i koja im je godina bila najuspješnija?
\end{itemize}
...
\begin{itemize}
  \item \texttt{d3.geoMercator()} i \texttt{path.bounds(feature)} --- prema kodu iz priručnika kolegija (poglavlje Projekcije) za učitavanje TopoJSON zapisa i stvaranje SVG putanja
...
\texttt{--muted} & \#5B6473 & osi, sekundarni tekst \\
\texttt{--no-data} & \#D8DDE5 & zemlje bez podataka na karti \\
\texttt{interpolateOrRd} & krem $\to$ tamnocrvena & gradijent choropleth karte \\
\texttt{schemeTableau10} & 10 različitih boja & po jedna boja po zemlji (stalna u svim prikazima) \\
\bottomrule
\end{tabular}
...
\begin{enumerate}
  ...
  \item raspoznatljivost --- paleta \texttt{schemeTableau10} sadrži 10 perceptivno različitih boja, dovoljno za top 8 zemalja i dvije dodatno označene
...
Identificirane osnovne funkcionalnosti:
...
\begin{itemize}
  \item sva četiri prikaza dijele isto reaktivno stanje --- promjena u kontrolnoj traci odmah se odražava
  \item klik na zemlju u bilo kojem prikazu osvježava sve ostale
  \item klizači imaju zaštitu od preklapanja --- najmanji razmak je dvije godine
  \item stupčasti se dijagram preraspoređuje uz postupnu animaciju (kašnjenje 35\,ms po rednom broju stupca)
\end{itemize}
...
U konačnoj su verziji dovršene sve osnovne funkcionalnosti. Dodatno je uvedeno:

\begin{itemize}
  \item prilagodba za mobilne uređaje --- prikazi su prilagođeni za širinu zaslona $< 980$\,px, klizač ima veći klizni element radi dodira (22\,px), a kontrole zauzimaju punu širinu
  \item najmanji razmak od dvije godine --- oba klizača dijele isti raspon, a vrijednost se pri pomicanju ograničava tako da se klizni elementi ne mogu preklopiti
  \item automatsko uključivanje označene zemlje u stupčasti i linijski prikaz ako nije u prikazanom skupu --- dodaje se na dno
\end{itemize}
...
Y-skala se ne ograničava na \texttt{maxPlace} ($\sim$25), nego se proširuje za \texttt{DNQ\_PAD} (dvije jedinice). Posebna vrijednost na osi, \texttt{dnqRow = maxPlace + 2}, prikazuje se oznakom ,,drugo'' umjesto broja. Točke za nastupe bez plasmana crtaju se na tom položaju, ostajući unutar grafa:
...
Pri svakoj promjeni metrike linije se ponovno iscrtavaju slijeva nadesno. Točke se prve postave (\texttt{POINT\_REVEAL\_MS} = 380\,ms), a zatim uz odgodu kreće animacija svojstva \texttt{stroke-dashoffset} (\texttt{LINE\_DELAY\_MS} = 320\,ms, \texttt{LINE\_DRAW\_MS} = 1300\,ms). Poziv \texttt{interrupt()} prekida tekuću animaciju ako korisnik brzo mijenja metrike:
...
\begin{itemize}
  ...
  \item konfiguracija alata ESLint i Prettier za provjeru stila i kvalitete koda
  \item pohrana sirovih podataka u repozitorij --- mapa \texttt{scripts/.cache/} sa CSV-om, TopoJSON zapisom i predmemorijom pregleda izuzeta je iz \texttt{.gitignore} datoteke tako da projekt radi i bez internetske veze
...
\begin{itemize}
  \item D3.js v7 (vizualizacijski sloj)
  \item TopoJSON (kompaktan geografski zapis)
  \item TypeScript (strogi način rada)
  \item Vite (razvojni poslužitelj i alat za izradu)
  \item Node.js (skripte za pripremu podataka)
  \item GitHub Actions (automatizacija izrade i objave)
  \item GitHub Pages (besplatno posluživanje)
\end{itemize}
...
Programski je kod strukturiran u modulima:

\begin{itemize}
  \item \texttt{src/main.ts} --- ulazna točka aplikacije
  \item \texttt{src/state.ts} --- reaktivno stanje
  \item \texttt{src/types.ts} --- domenski tipovi
  \item \texttt{src/metrics.ts} --- definicije metrika
  \item \texttt{src/tooltip.ts} --- zajednički skočni opis
  \item \texttt{src/charts/map.ts} --- choropleth karta
  \item \texttt{src/charts/bar.ts} --- stupčasti dijagram
  \item \texttt{src/charts/line.ts} --- linijski dijagram
  \item \texttt{src/charts/scatter.ts} --- dijagram raspršenja
  \item \texttt{scripts/build-data.js} --- priprema datoteke \texttt{entries.json}
  \item \texttt{scripts/fetch-youtube-views.js} --- dohvat broja pregleda
\end{itemize}

Ukupno približno 3100 redaka izvornog koda. Detaljan pregled koda dostupan je u javnom GitHub repozitoriju.
